#!/bin/bash
#SBATCH --job-name=raft-3frame-ours_augonly
...
echo "============================================"
echo " Task : 3-frame RAFT + affine aug on Sintel"
echo "        GT flow = compose(flow01, flow12)"
echo "        Input   = (img0, img2) + (img0, img2_aug)"
echo "        Loss    = (loss_orig + beta*loss_aug) / (1+beta)"
...
"$PYTHON_BIN" -u 3Frame_train_ours.py \
  --name raft-3frame-ours-sintel \
  --ckpt_dir checkpoints/3frame-ours-RAFT_thingsckpt \
  --stage sintel \
  --validation sintel \
